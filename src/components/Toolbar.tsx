...
/** Top toolbar: drag-out tools (note, folder), plus a "back to home" button while inside a folder. */
export function Toolbar({
  ...
  return (
    <div className="toolbar z-20 flex shrink-0 items-center gap-2 border-b border-neutral-200 bg-white px-3 py-2 sm:px-4">
      ...
      <button
        type="button"
        className="toolbar__folder-icon flex h-11 w-11 touch-none cursor-grab items-center justify-center rounded-lg border border-neutral-200 bg-neutral-50 hover:bg-neutral-100 active:cursor-grabbing"
        onPointerDown={onFolderToolPointerDown}
        onPointerMove={onFolderToolPointerMove}
        onPointerUp={onFolderToolPointerUp}
        aria-label="Crear carpeta (arrastra hacia el canvas)"
        title="Arrastra hacia el canvas para crear una carpeta"
      >
        <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
          <path
